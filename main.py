...
class User(UserMixin, db.Model):
    ...
    # The author refers to the autor property in the Blogpost class
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            flash("A user already exists with that email. Please login.")
            return redirect(url_for('login'))

        password = form.password.data
        hashed_salted_password = generate_password_hash(password, method='pbkdf2:sha256',
                                                        salt_length=8)
        new_user = User(
            name=form.name.data,
            email=email,
